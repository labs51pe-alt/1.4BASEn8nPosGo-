...
const App: React.FC = () => {
  ...
  const handleCheckout = async (method: string, payments: PaymentDetail[]) => {
      ...
      try {
          ...
          alert("Error de sincronización: " + (error?.message || "Error desconocido"));
      } finally {
          setIsSyncing(false);
      }
  };
  ...
  const handleCashAction = async (action: 'OPEN' | 'CLOSE' | 'IN' | 'OUT', amount: number, description: string) => {
      try {
          ...
          alert("Error en operación de caja: " + (err?.message || "Error desconocido"));
      }
  };
  ...
  const handleConfirmReception = async (purchase: Purchase) => {
      ...
      } catch (e: any) { alert("Error: " + e.message); } finally { setIsSyncing(false); }
  };
  ...
  return (
    <>
        <Layout currentView={view || ViewState.POS} onChangeView={setView} settings={settings} user={user} onLogout={handleLogout}>
              ...
              <PurchasesView 
                ...
                onProcessPurchase={async (pur) => { 
                  try { 
                    ...
                    const msg = e?.message || (typeof e === 'string' ? e : JSON.stringify(e)) || "Error desconocido";
                ...
                onAddSupplier={async (s) => { try { await StorageService.saveSupplier(s); await refreshAllData(); } catch(e:any){ alert("Error: "+(e?.message || (typeof e === 'string' ? e : JSON.stringify(e)) || "Error desconocido")); } }} 
            ...
            {view === ViewState.ADMIN && <AdminView transactions={transactions} products={products} shifts={shifts} movements={movements} settings={settings} onRefresh={refreshAllData} />}
            {view === ViewState.REPORTS && <ReportsView transactions={transactions} settings={settings} />}
            {view === ViewState.SETTINGS && <SettingsView settings={settings} onSaveSettings={async (s) => { try { await StorageService.saveSettings(s); await refreshAllData(); } catch(e:any){ alert("Error: "+(e?.message || (typeof e === 'string' ? e : JSON.stringify(e)) || "Error desconocido")); } }} />}
