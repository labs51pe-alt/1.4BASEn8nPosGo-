...
import { Trash2, CreditCard, Banknote, Minus, Plus, ShoppingBag, X, Zap, Smartphone, Check, Wand2, ChevronDown, DollarSign, Wallet, Percent, Coins } from 'lucide-react';
...
export const Cart: React.FC<CartProps> = ({ items, onUpdateQuantity, onRemoveItem, onCheckout, onClearCart, settings, onClose }) => {
  ...
  const [manualGlobalDiscount, setManualGlobalDiscount] = useState<string>('0');
  ...
  const cashDiscount = parseFloat(manualGlobalDiscount) || 0;
  
  const totalDiscount = itemDiscounts + cashDiscount;
    ...
    <div className="h-full flex flex-col bg-white relative">
      ...
      <div className="p-6 bg-white border-t border-slate-100 pb-24 lg:pb-8 shadow-[0_-10px_40px_rgba(0,0,0,0.02)]">
        <div className="space-y-2.5 mb-6">
            ...
            <div className="flex items-center justify-between py-2 border-y border-slate-50 group">
                <div className="flex items-center gap-2">
                    <div className="p-1.5 bg-rose-50 rounded-lg text-rose-500">
                        <Coins className="w-3 h-3" />
                    </div>
                    <span className="text-[10px] font-black text-rose-400 uppercase tracking-widest">Descuento (S/.)</span>
                </div>
                <div className="relative">
                    <span className="absolute left-2 top-1/2 -translate-y-1/2 text-rose-300 text-[10px] font-black">S/</span>
                    <input 
                        type="number" 
                        value={manualGlobalDiscount}
                        onChange={(e) => setManualGlobalDiscount(e.target.value)}
                        className="w-24 pl-6 pr-2 py-1.5 bg-rose-50/50 border border-rose-100 rounded-xl text-right font-black text-rose-600 outline-none focus:border-rose-400 transition-all text-xs"
                        placeholder="0.00"
                    />
                </div>
            </div>
