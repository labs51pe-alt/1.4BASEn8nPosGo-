...
import { ShoppingCart, Archive, BarChart2, ShoppingBag, LogOut, User, Settings, Rocket, ShieldAlert, Wallet, PieChart } from 'lucide-react';
  ...
  const MobileNavItem = ({ view, icon: Icon, label }: { view: ViewState; icon: any; label: string }) => (
    ...
    >
        <div className={`p-1.5 rounded-xl transition-all ${currentView === view ? 'bg-indigo-50 scale-110' : ''}`}>
            <Icon className={`w-5 h-5 ${currentView === view ? 'stroke-[2.5px]' : 'stroke-[2px]'}`} />
        </div>
    ...
    <div className="flex h-screen overflow-hidden relative bg-[#f8fafc]">
      ...
      <div className="hidden lg:flex w-24 bg-white/70 backdrop-blur-2xl border-r border-slate-200/60 flex-col items-center py-6 z-20 shadow-xl shadow-indigo-100/20 overflow-y-auto custom-scrollbar">
        ...
        <div className="flex-1 w-full px-3 flex flex-col">
          <NavItem view={ViewState.POS} icon={ShoppingCart} label="Ventas" colorClass="bg-indigo-50 text-indigo-600" />
          <NavItem view={ViewState.SHIFTS} icon={Wallet} label="Turnos" colorClass="bg-emerald-50 text-emerald-600" />
          <div className="h-px bg-slate-200 w-1/2 mx-auto my-3 opacity-50"></div>
          <NavItem view={ViewState.INVENTORY} icon={Archive} label="Stock" colorClass="bg-blue-50 text-blue-600" />
          <NavItem view={ViewState.PURCHASES} icon={ShoppingBag} label="Compras" colorClass="bg-amber-50 text-amber-600" />
          
          {(user?.role === 'admin' || isSuperAdmin) && (
            <>
             <div className="h-px bg-slate-200 w-1/2 mx-auto my-3 opacity-50"></div>
             <NavItem view={ViewState.ANALYTICS} icon={PieChart} label="Reportes" colorClass="bg-rose-50 text-rose-600" />
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 z-[40] pb-safe flex justify-between px-2 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
          <MobileNavItem view={ViewState.POS} icon={ShoppingCart} label="Ventas" />
          <MobileNavItem view={ViewState.SHIFTS} icon={Wallet} label="Turnos" />
          <MobileNavItem view={ViewState.INVENTORY} icon={Archive} label="Stock" />
          {(user?.role === 'admin' || isSuperAdmin) && (
              <MobileNavItem view={ViewState.ANALYTICS} icon={PieChart} label="Reportes" />
          )}
