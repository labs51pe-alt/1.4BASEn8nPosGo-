...
import { Lock, Wallet, LayoutGrid, List, ScanBarcode, Search, Layers, ShoppingBasket, Plus, AlertCircle, X, Tag, Check, Package, TrendingUp, Sparkles, Filter, Keyboard, ChevronRight, ShoppingBag, Box, Zap, Smartphone, CreditCard } from 'lucide-react';
...
export const POSView: React.FC<POSViewProps> = ({ 
  ...
  const shiftStats = useMemo(() => {
    if (!activeShift) return { total: 0, cash: 0, digital: 0, yape: 0, plin: 0, card: 0 };
    return {
        total: (activeShift.totalSalesCash || 0) + (activeShift.totalSalesDigital || 0),
        cash: activeShift.totalSalesCash || 0,
        digital: activeShift.totalSalesDigital || 0,
        yape: activeShift.totalSalesYape || 0,
        plin: activeShift.totalSalesPlin || 0,
        card: activeShift.totalSalesCard || 0
    };
  }, [activeShift]);
  ...
  return (
    <div className="h-full flex overflow-hidden bg-[#f8fafc]">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {/* Header POS Premium con Desglose */}
            <div className="bg-white border-b border-slate-200/60 px-8 py-5 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0 shadow-sm z-10">
                <div className="flex items-center gap-6 w-full sm:w-auto overflow-x-auto no-scrollbar">
                    <div className="flex items-center gap-4 bg-slate-50 px-5 py-3 rounded-2xl border border-slate-100 shadow-inner shrink-0">
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                         <div className="flex flex-col border-r border-slate-100 pr-4">
                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-wider leading-none mb-1">Ventas Turno</p>
                            <p className="text-2xl font-black text-slate-900 tracking-tight leading-none">{settings.currency}{shiftStats.total.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</p>
                        </div>
                        <div className="flex gap-3">
                             <div className="flex flex-col items-center">
                                 <Smartphone className="w-3 h-3 text-purple-400 mb-0.5"/>
                                 <span className="text-[9px] font-black text-purple-600">{settings.currency}{shiftStats.yape.toFixed(2)}</span>
                             </div>
                             <div className="flex flex-col items-center">
                                 <Zap className="w-3 h-3 text-cyan-400 fill-current mb-0.5"/>
                                 <span className="text-[9px] font-black text-cyan-600">{settings.currency}{shiftStats.plin.toFixed(2)}</span>
                             </div>
                             <div className="flex flex-col items-center">
                                 <CreditCard className="w-3 h-3 text-slate-400 mb-0.5"/>
                                 <span className="text-[9px] font-black text-slate-600">{settings.currency}{shiftStats.card.toFixed(2)}</span>
                             </div>
                        </div>
                    </div>
                </div>
